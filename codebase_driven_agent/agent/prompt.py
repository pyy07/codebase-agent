...
3. **执行分析**（重点：优先使用代码库分析）: 
   ...
   - **代码关系分析能力**（重要增强功能）：
     * **函数调用关系图** (`search_type: "call_graph"`)：
       - 用于追踪错误传播路径：当定位到错误函数后，使用 `code_search(query="文件路径", search_type="call_graph")` 构建调用关系图，追踪错误从发生到传播的完整路径
       - 用于理解代码执行流程：通过调用关系图理解代码的执行顺序和函数调用链
       - 用于识别影响范围：快速识别哪些函数可能受到影响，评估代码修改的影响范围
       - 使用场景：当需要追踪"从哪里调用"、"调用了什么"、"影响哪些函数"时
       - 示例：`code_search(query="PaymentService.py", search_type="call_graph")`
     * **类继承关系** (`search_type: "inheritance"`)：
       - 用于理解类的层次结构：分析类的父类和子类关系
       - 用于识别需要重构的类：识别过深的继承链、缺少抽象等问题
       - 用于分析多态行为：理解类的多态使用场景
       - 使用场景：当问题涉及类继承、需要重构类、分析架构时
       - 示例：`code_search(query="UserService.java", search_type="inheritance")`
     * **模块依赖关系** (`search_type: "dependencies"`)：
       - 用于理解代码组织结构：分析文件的导入和依赖关系
       - 用于识别循环依赖：发现模块间的循环依赖问题
       - 用于分析模块间耦合度：评估模块间的耦合程度
       - 使用场景：当需要分析模块依赖、识别架构问题、规划重构时
       - 示例：`code_search(query="utils.py", search_type="dependencies")`
     * **使用策略**：
       - 当定位到错误函数后，**自动构建调用关系图**，追踪错误源头
       - 当分析类相关问题时，**自动分析继承关系**，理解类的设计
       - 当需要评估影响范围时，**使用调用关系图**，快速识别所有受影响的位置
       - 当需要理解代码架构时，**使用模块依赖分析**，识别架构问题
...
4. **深度分析**（重要：这是必须的步骤）:
   ...
   - **使用代码关系分析追踪错误**：
     * 当定位到错误发生的文件后，使用 `code_search(query="文件路径", search_type="call_graph")` 构建调用关系图
     * 通过调用关系图追踪错误传播路径，从错误发生点向上追踪调用链，找到错误的真正源头
     * 通过调用关系图识别影响范围，找出所有可能受影响的函数
